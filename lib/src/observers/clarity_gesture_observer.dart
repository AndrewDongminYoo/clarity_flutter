...
class ClarityGestureObserver implements IObserver {
  ...
  static void _onPointerDown(PointerEvent details) => _instance?._handlePointerDown(details);

  static void _onPointerUp(PointerEvent details) => _instance?._handlePointerUp(details);

  static void _onPointerMove(PointerEvent details) => _instance?._handlePointerMove(details);

  static void _onPointerCancel(PointerEvent details) => _instance?._handlePointerCancel(details);
  ...
  void _initializeGlobalRouteCallback() {
    ...
    GestureBinding.instance.pointerRouter.addGlobalRoute(_globalPointerRouterHandler);
  }
  ...
  void _fireSingleClickEvent(PointerEvent event) {
    enqueueUserGestureEvent(
      Click(DateTime.now().millisecondsSinceEpoch, event.position.dx, event.position.dy, event.viewId),
    );
  }

  void _fireDoubleClickEvent(Offset clickedPosition, int pointerId) {
    enqueueUserGestureEvent(
      DoubleClick(DateTime.now().millisecondsSinceEpoch, pointerId, clickedPosition.dx, clickedPosition.dy),
    );
  }

  void _fireTouchStartEvent(Offset clickedPosition, int pointerId) {
    enqueueUserGestureEvent(
      TouchStart(DateTime.now().millisecondsSinceEpoch, pointerId, clickedPosition.dx, clickedPosition.dy),
    );
  }

  void _fireTouchMoveEvent(Offset clickedPosition, int pointerId) {
    enqueueUserGestureEvent(
      TouchMove(DateTime.now().millisecondsSinceEpoch, pointerId, clickedPosition.dx, clickedPosition.dy),
    );
  }

  void _fireTouchEndEvent(Offset clickedPosition, int pointerId) {
    enqueueUserGestureEvent(
      TouchEnd(DateTime.now().millisecondsSinceEpoch, pointerId, clickedPosition.dx, clickedPosition.dy),
    );
  }

  void _fireTouchCancelEvent(Offset clickedPosition, int pointerId) {
    enqueueUserGestureEvent(
      TouchCancel(DateTime.now().millisecondsSinceEpoch, pointerId, clickedPosition.dx, clickedPosition.dy),
    );
  }
  ...
  bool _checkWithinDurationRangeWithLastClick() {
    return _lastClickTime != null && DateTime.now().difference(_lastClickTime!) < kDoubleTapTimeout;
  }

  bool _checkWithinTouchSlopDistance(Offset currentPosition) {
    return _lastTouchStartPosition != null && (currentPosition - _lastTouchStartPosition!).distance <= kTouchSlop;
  }

  bool _checkWithinClickSlopDistance(Offset currentPosition) {
    return _lastClickPosition != null && (currentPosition - _lastClickPosition!).distance <= kDoubleTapSlop;
  }
...
class ClarityListenerWidget extends Listener {
  const ClarityListenerWidget({
    required PointerDownEventListener super.onPointerDown,
    required PointerMoveEventListener super.onPointerMove,
    required PointerUpEventListener super.onPointerUp,
    required PointerCancelEventListener super.onPointerCancel,
...
class ClarityRenderPointerListener extends RenderPointerListener {
  ClarityRenderPointerListener({
    required super.onPointerDown,
    required super.onPointerMove,
    required super.onPointerUp,
    required super.onPointerCancel,
    required super.behavior,
  });
