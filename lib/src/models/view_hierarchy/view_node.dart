...
class ViewNode implements IProtoModel<mutation_payload.ViewNodeDelta> {
  ...
  mutation_payload.ViewNodeDelta toProtobufInstance() {
    final node = mutation_payload.ViewNodeDelta(
      ...
      viewWidth: _getNewValueIfDifferentOrNull(previousFrameNode?.viewWidth, viewWidth),
      viewHeight: _getNewValueIfDifferentOrNull(previousFrameNode?.viewHeight, viewHeight),
      visible: _getNewValueIfDifferentOrNull(previousFrameNode?.visible, visible),
      text: _getNewValueIfDifferentOrNull(previousFrameNode?.text, text),
      isMasked: _getNewValueIfDifferentOrNull(previousFrameNode?.isMasked, isMasked),
...
class ViewNodeText {
  ...
  set text(String value) {
    _text = value.substring(0, min(value.length, ClarityConstants.viewNodeTextMaxLength));
    isFullText = value.length == text.length;
  }
}
