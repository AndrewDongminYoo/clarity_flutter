...
class Clarity {
  ...
  static bool setCustomUserId(String customUserId) => ClarityManager._setCustomUserId(customUserId);
  ...
  /// - This function should only be called on the UI Isolate.
  /// - We recommend NOT to set any Personally Identifiable Information value inside this field.
  ///
  /// Returns `true` if the tag was set successfully; otherwise `false`.
  static bool setCustomTag(String key, String value) => ClarityManager._setCustomTags(key, [value]);

  /// Sets a custom tag with multiple values for the current session.
  ///
  /// [key] must be a nonempty string with a maximum of 255 characters and can't consist only of whitespace.
  /// [values] must be a nonempty set of nonempty strings with a maximum of 255 characters each and can't consist only of whitespace.
  ///
  /// ## Remarks
  /// - This tag can be used to filter sessions on the Clarity dashboard.
  /// - This function should only be called on the UI Isolate.
  ...
  ///
  /// Returns `true` if the tag was set successfully; otherwise `false`.
  static bool setCustomTags(String key, Iterable<String> values) => ClarityManager._setCustomTags(key, values);
  ...
  static bool setCustomSessionId(String customSessionId) => ClarityManager._setCustomSessionId(customSessionId);
  ...
  static String? getCurrentSessionUrl() => ClarityManager._getCurrentSessionUrl();
  ...
  static bool sendCustomEvent(String value) => ClarityManager._sendCustomEvent(value);
  ...
  static bool setCurrentScreenName(String? screenName) => ClarityManager._setCurrentScreenName(screenName);

  /// Starts a new Clarity session, ending the current session if one is active.
  ///
  /// [callback] The callback to be invoked when the new Clarity session starts - the callback receives the new session ID as a string parameter.
  ///
  /// ## Remarks
  /// - This function should only be called on the UI Isolate.
  /// - The specified callback is guaranteed to run on the main isolate.
  ...
  static bool startNewSession(SessionStartedCallback callback) => ClarityManager._startNewSession(callback);
...
class ClarityManager {
  ...
  static bool initialize(BuildContext context, ClarityConfig clarityConfig) {
    return EntryPoint.run(
          () {
            ...
            unawaited(_init(clarityConfig));

            return true;
          },
          catchLogic: (e, _) {
            Logger.admin.out('Error initializing Clarity: initialize! message: $e');
          },
        ) ??
        false;
  }
  ...
  static bool _canStartClarity(BuildContext context) {
    if (!(Platform.isAndroid || Platform.isIOS)) {
      Logger.admin.out('Clarity supports only Android and iOS targets');
      return false;
    }

    final isObfuscated = Logger.admin.runtimeType.toString() != 'LevelLogger';
    if (isObfuscated) {
      Logger.admin.out('App is obfuscated');
      if (context.getElementForInheritedWidgetOfExactType<MediaQuery>() == null) {
  ...
  static Future<void> _init(ClarityConfig clarityConfig) async {
    if (_initializing) return;
    _initializing = true;

    final start = profileTimeAsync();
    start?.start('ClarityInit');

    await EntryPoint.runAsync<void>(
      () async {
        ...
        final projectConfig = await IngestService().getProjectConfigs(clarityConfig);
        if (projectConfig == null) {
          Logger.warn?.out('Failed to fetch Project configs, Clarity cannot start!');
          return;
        }
        ...
        if (projectConfig.lowEndDevices.disableRecordings && ProjectConfigUtils.isLowEndDevice()) {
          Logger.warn?.out('Clarity is deactivated on low-end devices.');
          return;
        }

        registry.registerItem(EnvRegistryKey.projectConfig, projectConfig);
        registry.registerItem(EnvRegistryKey.telemetryEnabled, projectConfig.reportUrl != null);
        registry.registerItem(EnvRegistryKey.rootIsolateToken, RootIsolateToken.instance);
        ...
        if (_onSessionStartedOrResumedCallback != null) {
          _sessionManager!.setOnSessionStartedOrResumedCallback(_onSessionStartedOrResumedCallback!);
        }

        if (_startNewSessionCallback != null) {
          _captureManager!.startNewSession(_startNewSessionCallback!);
        }
        ...
        for (final entry in _customTags.entries) {
          _sessionManager!.setCustomTags(entry.key, entry.value);
        }
  ...
  static bool _setCustomUserId(String customUserId) =>
      _setCustomTags(ClarityConstants.customUserIdVariableLabel, [customUserId]);

  static bool _setCustomTags(String key, Iterable<String> values) {
    return EntryPoint.run(
          () {
            final uniqueValues = values.toSet();
            if (uniqueValues.isEmpty) {
              (_clarityStarted ? Logger.error : Logger.admin)?.out('Custom tag value cannot be empty.');
              return false;
            }

            if (key.trim().isEmpty || uniqueValues.any((v) => v.trim().isEmpty)) {
              (_clarityStarted ? Logger.error : Logger.admin)?.out('Custom tag key and value cannot be blank.');
              return false;
            }

            if (key.length > ClarityConstants.customTagMaxLength ||
                uniqueValues.any((v) => v.length > ClarityConstants.customTagMaxLength)) {
            ...
            Logger.info?.out("Setting custom tag: $key = ${uniqueValues.join(',')}");

            if (_sessionManager == null) {
              _customTags[key] = uniqueValues;
            } else {
              _sessionManager!.setCustomTags(key, uniqueValues);
            }
          ...
          catchLogic: (e, st) {
            (_clarityStarted ? Logger.error : Logger.admin)?.out(
              'Error setting custom tag! Type: ${e.runtimeType} message: $e',
              stackTrace: st,
            );
            TelemetryTracker.instance?.trackError(ErrorType.SettingCustomTag, e.toString(), st);
          },
        ) ??
        false;
  }

  static bool _setCustomSessionId(String customSessionId) =>
      _setCustomTags(ClarityConstants.customSessionIdVariableLabel, [customSessionId]);
  ...
  static bool _setOnSessionStartedCallback(SessionStartedCallback callback) {
    return EntryPoint.run(
          ...
          catchLogic: (e, st) {
            (_clarityStarted ? Logger.error : Logger.admin)?.out(
              'Error setting on session started callback! Type: ${e.runtimeType} message: $e',
              stackTrace: st,
            );
            TelemetryTracker.instance?.trackError(ErrorType.SettingOnSessionStartedCallback, e.toString(), st);
          },
        ) ??
        false;
  }

  static bool _startNewSession(SessionStartedCallback callback) {
    return EntryPoint.run(
          () {
            if (_captureManager == null) {
              _startNewSessionCallback = callback;
            } else {
              _captureManager!.startNewSession(callback);
            }
          ...
          catchLogic: (e, st) {
            (_clarityStarted ? Logger.error : Logger.admin)?.out(
              'Error starting new session! Type: ${e.runtimeType} message: $e',
              stackTrace: st,
            );
            TelemetryTracker.instance?.trackError(ErrorType.StartNewClaritySession, e.toString(), st);
          },
        ) ??
        false;
  }
  ...
  static String? _getCurrentSessionUrl() {
    return EntryPoint.run<String?>(
      ...
      catchLogic: (e, st) {
        (_clarityStarted ? Logger.error : Logger.admin)?.out(
          'Error getting current session url! Type: ${e.runtimeType} message: $e',
          stackTrace: st,
        );
        TelemetryTracker.instance?.trackError(ErrorType.GettingCurrentSessionUrl, e.toString(), st);
      },
    );
  }
    ...
    return EntryPoint.run(
          () {
            if (value.trim().isEmpty) {
              (_clarityStarted ? Logger.error : Logger.admin)?.out('Custom event value cannot be blank.');
              return false;
            }
          ...
          catchLogic: (e, st) {
            (_clarityStarted ? Logger.error : Logger.admin)?.out(
              'Error sending custom event! Type: ${e.runtimeType} message: $e',
              stackTrace: st,
            );
            TelemetryTracker.instance?.trackError(ErrorType.SendingCustomEvent, e.toString(), st);
          },
        ) ??
        false;
  }
    ...
    return EntryPoint.run(
          () {
            if (screenName != null && screenName.trim().isEmpty) {
              (_clarityStarted ? Logger.error : Logger.admin)?.out('Current screen name cannot be blank.');
              return false;
            }

            if (screenName != null && screenName.length > ClarityConstants.currentScreenNameMaxLength) {
              (_clarityStarted ? Logger.error : Logger.admin)?.out(
          ...
          catchLogic: (e, st) {
            (_clarityStarted ? Logger.error : Logger.admin)?.out(
              'Error setting current screen name! Type: ${e.runtimeType} message: $e',
              stackTrace: st,
            );
            TelemetryTracker.instance?.trackError(ErrorType.SettingCurrentScreenName, e.toString(), st);
          },
        ) ??
        false;
  }
  ...
  static bool _pause() {
    return EntryPoint.run(
          ...
          catchLogic: (e, st) {
            (_clarityStarted ? Logger.error : Logger.admin)?.out(
              'Error pausing Clarity! Type: ${e.runtimeType} message: $e',
              stackTrace: st,
            );
            TelemetryTracker.instance?.trackError(ErrorType.PausingClarity, e.toString(), st);
          },
        ) ??
        false;
  }
  ...
  static bool _resume() {
    return EntryPoint.run(
          ...
          catchLogic: (e, st) {
            (_clarityStarted ? Logger.error : Logger.admin)?.out(
              'Error resuming Clarity! Type: ${e.runtimeType} message: $e',
              stackTrace: st,
            );
            TelemetryTracker.instance?.trackError(ErrorType.ResumingClarity, e.toString(), st);
          },
        ) ??
        false;
  }
