...
class SessionRepository {
  ...
  Future<void> createSessionPayload(PayloadMetadata payloadMetadata) async {
    ...
    await _currentSessionStores!.payloadStore.writeToFile(file, '', WriteMode.overwrite);
    file = getPayloadFileName(payloadMetadata, PayloadDataType.playback);
    await _currentSessionStores!.payloadStore.writeToFile(file, '', WriteMode.overwrite);
  }

  Future<void> appendEventToSessionPayload(PayloadMetadata payloadMetadata, SessionEvent event) async {
    final type = event is AnalyticsEvent ? PayloadDataType.analytics : PayloadDataType.playback;

    final fileName = getPayloadFileName(payloadMetadata, type);
    await appendEvent(_currentSessionStores!.payloadStore, fileName, event, payloadMetadata.pageStartTime);
  }

  Future<void> appendEvent(FileStore store, String fileName, SessionEvent event, int pageStartTime) =>
      store.writeToFile(fileName, '${event.serialize(pageStartTime)}\n', WriteMode.append);
  ...
  Future<void> deleteSessionPayload(PayloadMetadata payloadMetadata) async {
    ...
    var payloadFileName = getPayloadFileName(payloadMetadata, PayloadDataType.analytics);
    await _currentSessionStores!.payloadStore.deleteFile(payloadFileName);
    payloadFileName = getPayloadFileName(payloadMetadata, PayloadDataType.playback);
    await _currentSessionStores!.payloadStore.deleteFile(payloadFileName);
  }
  ...
  Future<void> deleteResidualSessionData() async {
    ...
    await clarityStore.deleteDirectoriesModifiedBeforeTimestampRecursively(DateTime.now().microsecondsSinceEpoch);
  }
  ...
  Future<List<String>> getPayloadSerializedEvents(PayloadMetadata payloadMetadata, PayloadDataType type) async {
    final eventsFile = getPayloadFileName(payloadMetadata, type);
    final fileContent = await _currentSessionStores!.payloadStore.readFileToString(eventsFile);
  ...
  static String getPayloadFileName(PayloadMetadata payloadMetadata, PayloadDataType type) {
...
class SessionStores {
  SessionStores(Directory cacheDir, SessionMetadata sessionMetadata)
    : payloadStore = FileStore(
        cacheDir,
        FileUtils.concat([SessionRepository.getSessionFolderName(sessionMetadata), ClarityConstants.payloadsDirectory]),
      ),
