...
mixin TelemetryHandler {
  ...
  void enqueueTelemetry(TelemetryItem item) {
    ...
    _periodicFlusher ??= Timer.periodic(Duration(seconds: telemetryAggregateDurationInSeconds), _flushAndClear);
  }

  void _flushAndClear(Timer timer) {
    if (_errors.isEmpty && _metrics.isEmpty) {
      // Cancel the timer and restart when new data is added
      Logger.verbose?.out('Pausing Telemetry periodic check until new telemetry is generated');
      timer.cancel();
    ...
    EntryPoint.run<void>(
      () {
        final container = TelemetryContainer(_metrics.values.toList(), _errors.toList());
        _metrics.clear();
        _errors.clear();
        unawaited(processTelemetry(container));
      },
      catchLogic: (e, st) {
        // No Telemetry tracking for Telemetry related issues to avoid infinite queueing
        Logger.error?.out('Failed to process telemetry $e', stackTrace: st);
      },
    );
  }
