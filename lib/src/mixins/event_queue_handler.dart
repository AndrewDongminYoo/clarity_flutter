...
mixin EventQueueHandler {
  ...
  Future<void> enqueueEvent(Event event) async {
    ...
    Logger.verbose?.out('Event of type ${event.runtimeType} is enqueued for processing');
    ...
    if (!_currentlyProcessing) {
      ...
      while (_eventsQueue.isNotEmpty) {
        ...
        await EntryPoint.runAsync<void>(
          ...
          catchLogic: (e, st) {
            Logger.error?.out('Failed to process event $event $e', stackTrace: st);
  ...
  Future<void> processEvent(Event event) {
    throw UnimplementedError('ProcessEvent needs to be implemented in subclass!');
  }
