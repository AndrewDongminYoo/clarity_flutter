...
class DataUtils {
  ...
  static String md5HashBase64(List<int> input) {
    ...
    msg.addAll(Uint8List(8)..buffer.asByteData().setUint64(0, bitLength, Endian.little));

    // Process the message in successive 512-bit chunks
    for (var i = 0; i < msg.length; i += 64) {
      final chunk = Uint8List.fromList(msg.sublist(i, i + 64));
      final m = List<int>.generate(16, (j) => chunk.buffer.asByteData().getUint32(j * 4, Endian.little));
      ...
      for (var j = 0; j < 64; j++) {
        ...
        d = c;
        c = b;
        b = b + ((a + f + _k[j] + m[g]) << _s[j] | (a + f + _k[j] + m[g]) >> (32 - _s[j]));
        a = temp;
      }
